...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    length_1 = len(typed_words)
    length_2 = len(reference_words)
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    current_difference = limit + 1
    lowest_word = []
    if typed_word in word_list:
        return typed_word
    for word in word_list:
        if current_difference > diff_function(typed_word, word, limit):
            current_difference = diff_function(typed_word, word, limit)
            lowest_word = word 
    if current_difference > limit:
        return typed_word
    else:
        return lowest_word
    # END PROBLEM 5
...
def sphinx_swaps(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def helper(h_start, h_goal, length):
        if length > limit:
            return limit + 1
        if len(h_start) == 0 and len(h_goal) == 0:
            return length
        elif len(h_start) == 0  or len(h_goal) == 0:
            return helper(h_start[1:], h_goal[1:], length + 1)
        elif h_start[0] == h_goal[0]:
            return helper(h_start[1:], h_goal[1:], length)
        else:
            return helper(h_start[1:], h_goal[1:], length + 1)
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    for i in range(len(sofar)):
        if sofar[i] == prompt[i]:
            ...
            break
    correct_fraction = correct_words / total
    message ={'id': user_id, 'progress': correct_fraction}
    upload(message)
...
def time_per_word(words, times_per_player):
    ...
    for player in times_per_player:
        tpw = [player[i]-player[i-1] for i in range(1,len(player))]
        time_taken.append(tpw)
    return match(words,time_taken)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for w in word_indices:
        for p in player_indices:
            general_time = time(match, p, w)
            if p == 0:
                i = 0
                minimum_time = time(match, p, w)
            if general_time < minimum_time:
                minimum_time = general_time
                i = p
        fw[i] += [word_at(match, w)]
    return fw
    # END PROBLEM 10
