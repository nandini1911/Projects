...
def hefty_hogs(player_score, opponent_score):
    ...
    if opponent_score != 0:
        ...
        while opponent_score > 0:
            k = opponent_score % 10
            added_score = (digit_fn(k))(added_score)
            opponent_score = opponent_score // 10
    return (added_score % 30)
...
def make_averaged(original_function, total_samples=1000):
    ...
    def execute(*args):
        x = 0
        apply_number = total_samples
        while apply_number > 0:
            x += original_function(*args)
            apply_number -= 1
        divided_value = x / total_samples
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    1
    """
    other_score = make_averaged(roll_dice, total_samples) 
    i = 1 
    score_2 = 0
    best_roll = 0
    while i <= 10:
        current_score = other_score(i, dice)
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    other_score = hefty_hogs(score, opponent_score) + score
    if hog_pile(other_score, opponent_score) or hefty_hogs(score, opponent_score) >= threshold:
            return 0
    return num_rolls
