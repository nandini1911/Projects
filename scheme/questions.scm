...
(define (enumerate s)
  ...
  ;helper function with the parameter of index 
  ;list (index (car s))
  ;helper can keep track of the index + the original list
)
...
(define (merge inorder? list1 list2)
  ; BEGIN PROBLEM 16
  (cond 
    ((= 0 (length list1)) list2)
    ((= 0 (length list2)) list1)
    ((inorder? (car list1) (car list2)) (cons (car list1) (merge inorder? (cdr list1) list2)))
    (else (inorder? (car list2) (car list1)) (cons (car list2) (merge inorder? list1 (cdr list2))))
  )
)
  ; END PROBLEM 16
