...
def do_lambda_form(expressions, env):
    ...
    #lambda procedure class, look at the class of the lambda procedure, return an instance of the lambda class 
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    value = None
    if expressions.__len__() == 0:
        value = True
    while expressions != nil:
        if expressions.rest == nil:
            value = scheme_eval(expressions.first, env, tail = True)
        else:
            value = scheme_eval(expressions.first, env, tail = False)
